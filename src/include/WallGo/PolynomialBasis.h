...
static inline double T(uint32_t n, double x) { return std::cos(n* std::acos(x)); }
...
static inline double pPar_to_rhoPar(double pPar) { return 1.0 - 2.0 * std::exp(-pPar); }

// Calculate "physical" momentum components p_z and p_par, in units of T, by inverting definitions of rho_z and rho_par
static inline double rhoZ_to_pZ(double rho_z) { return 2.0 * std::atanh(rho_z); }
static inline double rhoPar_to_pPar(double rho_par) { return -std::log(0.5 * (1 - rho_par)); }

// Calculate Tm(rhoZ) Tn(rhoPar) for a given input momenta
static inline double TmTn(uint32_t m, uint32_t n, double rhoZ, double rhoPar)
{
